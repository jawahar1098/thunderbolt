<script>
  ...
  function top_10_day_location_between_dates() {
    gotResult = false;
    const top_10_day_location_between_dates = new FormData();
    top_10_day_location_between_dates.append("number", number);
    if (startDate)
      top_10_day_location_between_dates.append("from_date", startDate);
    if (endDate) top_10_day_location_between_dates.append("to_date", endDate);
    top_10_day_location_between_dates.append("mode", "Top10DayLocWithDate");
    showProgress = true;
    // try {
    //   const response = await fetch(`${basepath()}/location`, {
    //     method: "POST",
    //     body: top_10_day_location_between_dates,
    //   });
    const url = `${basepath()}/location`;
    postRequest(fetch,url,top_10_day_location_between_dates)
    .then(data => {
  ...
  afterUpdate(() => {
    if (number != propData.number) {
      ...
      top_10_day_location_between_dates();
      number = "";
      startDate = "";
      endDate = "";
    }
...
{#if gotResult}
  <div class="flex justify-center mt-3">
    <h2 class="heading">DAY LOCATION - TOP 10</h2>
  </div>
