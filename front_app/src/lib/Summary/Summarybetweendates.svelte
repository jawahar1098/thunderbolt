<script>
  ...
  let pro_data = []
	...
	// summary_dates flask endpoint, completed
	function summaryDatesForm() {
	  gotResult = false;
	  showProgress = true;
	  const summaryDatesForm = new FormData();
	  summaryDatesForm.append("number", number);
	  if (startDate) summaryDatesForm.append("fromdate", startDate);
	  if (endDate) summaryDatesForm.append("todate", endDate);
	  summaryDatesForm.append("mode", "summaryBetweenDates");
	  showProgress = true;
	  // try {
		...
		//   {
		// 	method: "POST",
		// 	body: summaryDatesForm,
		//   }
		// );
    const url = `${basepath()}/summary_of_mobile`;
    postRequest(fetch,url,summaryDatesForm)
    .then(data => {
  

		  if (data.data_dict === "Not data Matched") {
			  console.log('adsfasfffffffffffffffffffffff')
			  showProgress = false;
			  datafound = "No Data Matched In Database";
			} else {
        pro_data = data.data_dict.data_dict
...
{#if gotResult}
  ...
  <div class="flex justify-center mt-1">
    <h4 class="heading">SUMMARY NEW - {propData.number}</h4>
  </div>
