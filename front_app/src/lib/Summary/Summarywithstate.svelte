<script>
  ...
  let dest_num = "";
  let parammode = "";
  let HomeContentHeading = "withinstate";
  let final_result = [];
  let columnFilters = {};
  let filteredResults = [];
  const dispatch = createEventDispatcher();

  summary_within_state();
  // Refer direct_contacts flask endpoint, completed
  function summary_within_state() {
    gotResult = false;
    const summary_within_state = new FormData();
    summary_within_state.append("number", number);
    if (startDate) summary_within_state.append("fromdate", startDate);
    if (endDate) summary_within_state.append("todate", endDate);
    summary_within_state.append("state", state);
    summary_within_state.append("mode", "SummaryWithState");
    summary_within_state.append("page", currentPage);
    summary_within_state.append("items_per_page", itemsPerPage);
    showProgress = true;
    // try {
    //   const response = await fetch(`${basepath()}/summary_for_state`, {
    //     method: "POST",
    //     body: summary_within_state,
    //   });

    const url = `${basepath()}/summary_for_state`;
    postRequest(fetch,url,summary_within_state)
    .then(data => {
      ...
      <tbody>
        ...
        {#each filteredResults as item}
          <tr class="border-b hover:bg-gray-100 dark:border-neutral-500">
            ...
            <td
              ><a
                href="/cdat/hyperlink?number={number}&mode=call_in&dest_number={item[
                  'destination_number'
            ...
            <td
              ><a
                href="/cdat/hyperlink?number={number}&mode=call_out&dest_number={item[
                  'destination_number'
                ]}&fromdate={startDate}&todate={endDate}"
            ...
            <td
              ><a
                href="/cdat/hyperlink?number={number}&mode=total_calls&dest_number={item[
                  'destination_number'
                ]}&fromdate={startDate}&todate={endDate}"
