<script>
  ...
  // contacts_of_mobileNumber MOBILE HANDSET SEARCH,,mobile_handset_analysis flask endpoint, completed
    ...
    gotResult = false;
    const date_target = new FormData();
    date_target.append("number", number);
    date_target.append("mode", "date_target");
    ...
    postRequest(fetch,url,date_target)
    .then(data => {
            ...
            <tr>
              ...
              <th scope="col" style="width: 5%;">
                ...
                <br /><input
                  ...
                  placeholder="search"
                  on:input={(event) =>
                    handleColumnSearch("mercant_phone", event)}
                />
              </th>
