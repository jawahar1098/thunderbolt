<script>
  ...
  localStorage.setItem("pdf_loop", '1');
...
export async function updateRecord(id, status) {
  ...
  function clearCheckboxes1() {
    ...
    let state = [];
    async function unique_state(id){
      const response = await fetch(basepath() + '/unique_state', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
        ...
        const data = await response.json();
        if (response.ok) {
          const states = [...new Set(data.states.map(user => user))];
          state = states;
          console.log(state)
        }
    }

    // const id  = cases.filter((user) => user.token && user.token.includes("_CO"))
    // console.log(cases,'///////////////')
    //   if (id.length !== 0){
    //     for (let i = 0; i < id.length; i++) {
    //       const ids = id[i];
    //       unique_state(id[i]["_id"])
    //   }
    // }
  
    async function view_annexure(id,st) {
    localStorage.setItem("pdf_loop", '1');
      const url = `/ticketing/Annexure?user=${id}?st=${st}`;
  ...
  localStorage.setItem("pdf_loop", '1');
    ...
    const response = await fetch(basepath()+"/upload-pdf", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
...
function clearCheckboxes11() {
    selected_id = '';
    console.log("requesttypes_updated: ", selected_id)
    const checkboxes1 = document.querySelectorAll('.annexe_check');
    checkboxes1.forEach((checkbox1) => {
      checkbox1.checked = false;
    });
...
.no-data {
    ...
    color: transparent;
    background: linear-gradient(to right, #d21c3a, #f2fcfe); /* Gradient colors */
  ...
  @keyframes bounce {
    ...
    .page_border {
      ...
      /* position: relative; */
      background:linear-gradient(10deg, #eeeded, #b2d8e9);
    ...
    <thead class="table-dark" id="myhead">
     ...
     {#each cases as user}
      <tr>
        ...
        {#if userDetails.role === "Mailer" && user.pending === 'Mail Under Process'}
    {#if user.requestcategory === "Data Request"}
      {#if user.token.includes("_CO")
        || user.requesttype === 'IMEI CDR'}
            
                <button class="btn btn-link dropdown-toggle mt-2" data-bs-placement="top" title='View Annexure' on:click={()=> unique_state(user._id)}  type="button" id="dropdownMenuButton1" data-bs-toggle="dropdown" aria-expanded="false"><i class="bi bi-three-dots-vertical"></i></button>
                <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1" style="background-color: white;">
                  <li>
                    <button class="btn btn-outline-warning" style="margin-left:15px !important" on:click={Annexe_id(user.requesttype)} ><i class="bi bi-cloud-arrow-down"></i></button>
                  </li>
                  {#if user.token.includes("_CO")}
                    {#if user.requesttype === "CDR"}
                    <div class="d-flex">
                    <input class="annexe_check" type="checkbox" bind:group={selected_id} value={user._id} />
                      <li>
                          <a class="dropdown-item" on:click={view_annexure(user._id,'All')}>Annexure-All</a>
                      </li>
                    </div>
                      ...
                      <div class="d-flex">
                        <input class="annexe_check" type="checkbox" bind:group={selected_id} value={user._id} />
                        <li>
                          <a class="dropdown-item" on:click={view_annexure(user._id,st)}>Annexure-{st}</a>
                          </li>
                          </div>
                      {/each}
                    {/if}
                    {#if user.requesttype !== "CDR" && user.requesttype !== "IMEI CDR" && user.requesttype === "TOWER CDR" || user.requesttype === "CAF" || user.requesttype === "SDR"
                    || user.requesttype === 'IPDR' && user.subtypes === 'Phone' || user.requesttype === 'RH' && user.subtypes === 'Phone'}
                      {#each state as st}
                      <div class="d-flex">
                        <input class="annexe_check" type="checkbox" bind:group={selected_id} value={user._id} />
                          <li>
                  ...
                  {:else if user.requesttype === "IMEI CDR" }
                  <div class="d-flex">
                    <input class="annexe_check" type="checkbox" bind:group={selected_id} value={user._id} />
                      <li>
                        <a class="dropdown-item" on:click={view_annexure(user._id,'All')}>View Annexure</a>
                      </li>
                  </div>
                  {:else}
                  <button class="btn btn-link dropdown-toggle mt-2" style="color:red !important" type="button" id="dropdownMenuButton1" data-bs-toggle="dropdown" aria-expanded="false" disabled><i class="bi bi-three-dots-vertical"></i></button>
                  {/if}
                  </ul>
                  {:else}
                  <button class="btn btn-link dropdown-toggle mt-2" style="color:red !important" type="button" id="dropdownMenuButton1" data-bs-toggle="dropdown" aria-expanded="false" disabled><i class="bi bi-three-dots-vertical"></i></button>
        ...
        <button class="btn btn-outline-danger mt-2 {user.pending === 'Fetch_Pending_Data' || user.pending === 'Approval Pending' || user.pending === 'Analysis Note Raised' || user.pending === 'Inspector Approval Pending' || user.pending === 'DSP Approval Pending'  || user.pending === 'SP Approval Pending' ? '' : 'hide'}" type="button" data-bs-toggle="modal" data-bs-target="#myModal3{user._id}">
            ...
            <div class="modal fade" id="myModal3{user._id}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
              <div class="modal-dialog" style="width: 30%;">
                <div class="modal-content">
                  <div class="modal-header">
                    <h3 style="font-weight:bold;">Are you sure to delete this Ticket?</h3>
                  </div>
            ...
            <button on:click={()=> fetch_officers('dsp')} data-bs-placement="top" title='Send to DSP' class= "ml-2 btn btn-outline-success {user.pending=== "Ins Approval Pending" 
                ...
                <div class="modal fade" id="myModal4{user._id}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" data-bs-backdrop="static">
                  <div class="modal-dialog" style="width: 30%;">
                    <div class="modal-content">
                      <div class="modal-header">
                        <h3 class="text-wrap" style="font-weight:bold;">Are you sure to Send this ticket to Other Superior?</h3>
                      </div>
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            {#if user.requestcategory === 'Data Request' && (user.requesttype === 'CDR' || user.subtypes === 'Phone')}
              <button class="btn btn-outline-warning" style="margin-bottom: 20px;margin-left:55rem" on:click={()=> autoresult(user._id)}>Automation Details</button>
              <button class="btn btn-outline-primary" style="margin-bottom: 20px; justify-content: center;" on:click={autoclose}>Close</button>
              {#if auto_result !== ''}
  ...
  {/if}  
      ...
      <div class="tab-pane fade" id="analysis-request" role="tabpanel" aria-labelledby="analysis-request-tab">
        <div class="search_bar">
          ...
          <h2 style="font-size: 2rem;"></h2>
          {/if}
          <div class="search btn-group" style='margin-bottom: 10px;margin-right: 30px'>
            ...
            <button data-bs-placement="top" title='Send to DSP' class= "ml-2 btn btn-outline-success {user.pending=== "Ins Approval Pending" 
                ...
                <div class="modal fade" id="myModal4{user._id}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" data-bs-backdrop="static">
                  <div class="modal-dialog" style="width: 30%;">
                    <div class="modal-content">
                      <div class="modal-header">
                        <h3 class="text-wrap" style="font-weight:bold;">Are you sure to Send this ticket to Superior?</h3>
                      </div>
                      <div class="modal-body text-left"> 
                        <div class="container">
                          <div class="row">
